...
def insert_rows(cur, conn, schema, row):
    try:
        ...
        logger.error(f"Error inserting row with Video_ID: {row[video_id]} into {schema}.{table}")
        raise e
...
def update_rows(cur, conn, schema, row):
    try:
        ...
        logger.error(f"Error updating row with Video_ID: {row[video_id]} in {schema}.{table}")
        raise e
...
def delete_rows(cur, conn, schema, ids_to_delete):
    try:
        ...
        logger.error(f"Error deleting rows with Video_IDs: {ids_to_delete} from {schema}.{table}")
        raise e