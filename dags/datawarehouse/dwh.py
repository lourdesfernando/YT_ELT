from datawarehouse.data_utils import get_connection_cursor, close_connection_cursor, create_schema, create_table, get_video_ids
...
def staging_table():
    ...
    try:
        ...
        logger.error(f"Error updating table {schema}.{table}: {e}")
        raise e
    finally:
        if conn and cur:
            close_connection_cursor(conn, cur)
